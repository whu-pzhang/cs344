                              ...
                              const float *const filter, const int filterWidth)
{
  ...
  for (size_t i = 0; i < filterWidth; ++i)
    ...
    {
      int x_img = x - halfFilterWidth + i;
      int y_img = y - halfFilterWidth + j;
      // if (x_img < 0 || y_img < 0 || x_img > numCols - 1 || y_img > numRows - 1)
      //   continue;
      if (x_img < 0)
      {
        x_img = 0;
      ...
      float image_value = (float)inputChannel[x_img + y_img * numCols];
      result += image_value * filter[i + j * filterWidth];
    }
                                 ...
                                 unsigned char *const blueChannel)
{
  ...
  int index = x + y * numCols;
  const uchar4 RGBAPixels = inputImageRGBA[index];
  redChannel[index] = RGBAPixels.x;
  greenChannel[index] = RGBAPixels.y;
  blueChannel[index] = RGBAPixels.z;
}
